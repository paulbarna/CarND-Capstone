...
class Bridge(object):
    def __init__(self, conf, server):
        ...
        self.subscribers = [rospy.Subscriber(e.topic,
                                             TYPE[e.type],
                                             self.callbacks[e.topic])
                            for e in conf.subscribers]

        self.publishers = {e.name: rospy.Publisher(e.topic,
                                                   TYPE[e.type],
                                                   queue_size=1)
    ...
    def publish_controls(self, data):
        steering, throttle, brake = data['steering_angle'], \
                                    data['throttle'], \
                                    data['brake']
    ...
    def publish_traffic(self, data):
        ...
        header.stamp = rospy.Time.now()
        header.frame_id = '/world'
        lights.lights = [self.create_light(*e) for e in zip(x, y, z,
                                                            yaw, status)]
    ...
    def callback_path(self, data):
        ...
        self.server('drawline', data={'next_x': x_values,
                                      'next_y': y_values,
                                      'next_z': z_values})
